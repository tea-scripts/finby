...
describe('TabBarIcon', () => {
  it('shows the filled icon when focused', async () => {
    await render(<TabBarIcon outline="grid-outline" filled="grid" focused color="#1d6ef5" />);
    expect(screen.getByText('grid')).toBeTruthy();
    expect(screen.getByTestId('tab-bar-icon')).toBeTruthy();
  });

  it('shows the outline icon when not focused', async () => {
    await render(<TabBarIcon outline="grid-outline" filled="grid" focused={false} color="#8da3c0" />);
